...
Replacement apps for Wii U kiosk. These are new homebrew versions of these apps, that **don't** use any Nintendo code or assets.
...
Note: 
* Titles may show up as "???" on the Wii U. Do not delete them. 
* They don't need a `content/` folder or boot images.

For reverse-engineering notes on how the kiosk menu uses these titles, see [HowKioskMenuWorks.MD](docs/HowKioskMenuWorks.MD).


Read about [AI usage](#ai).
...
2. Customize `meta/` if needed (demo lists and videos are not included in the release).
...
Nintendo dumps, original RPX binaries, or Ghidra outputs.
