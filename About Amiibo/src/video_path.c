...
append_meta_video(int index, char *out, size_t outSize)
{
   ...
   ACPInitialize();
   bool ok = ACPGetTitleMetaDir(ABOUT_AMIIBO_TID, metaDir, sizeof(metaDir)) == ACP_RESULT_SUCCESS;
   ACPFinalize();
   if (!ok) {
      return false;
   }

   snprintf(out, outSize, "%s/AboutAmiibo%02d.mp4", metaDir, index);
   return true;
}
...
{
   snprintf(out, outSize, "/vol/content/AboutAmiibo%02d.mp4", index);
   return true;
}
...
VideoPath_BuildIndex(int index, char *out, size_t outSize)
{
   ...
   if (index < 0 || index >= ABOUT_AMIIBO_VIDEO_COUNT) {
      return false;
   }
...
parse_index_arg(const char *arg)
{
   ...
   if (end && *end == '\0' && value >= 0 && value < ABOUT_AMIIBO_VIDEO_COUNT) {
      return (int)value;
   }

   const char *prefix = "AboutAmiibo";
   ...
   if (end && (*end == '\0' || strcmp(end, ".mp4") == 0) &&
       value >= 0 && value < ABOUT_AMIIBO_VIDEO_COUNT) {
      return (int)value;
   }
